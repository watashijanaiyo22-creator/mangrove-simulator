...
5. 元の8テスト維持。閉鎖系mass drift≤初期massの1e-5、静水max speed≤1e-5m/s、短期CPU/GPU absolute error≤1e-4（超える場合は成分単位と原因を報告）。補正ledgerゼロを合格条件とし、clampで失敗を隠さない。
...
12. 根あり/なしで波高RMS、流速RMS、底面せん断応力を同じ帯・同じ定常時間窓で計測。Cdを結果に合わせて調整しない。文献根拠とモデルの適用範囲を報告。
