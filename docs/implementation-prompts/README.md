...
実行用入口: **00-orchestrator.md**。このファイルを主担当へ渡し、同フォルダーの01–05を読み込ませる。今回はプロンプト作成のみ。worker/worktreeの起動はまだ行っていない。
...
- 05: 事前検証・段階ゲート・成分別受入・証拠保全・中断再開（v2）
...
改訂v2: 機能並列化前のP0/P1を追加。循環依存を排除し、根抽出失敗の手動代替、規則波の等価波高の名称訂正、step単位の同期、gate別新worktree、証拠のGit保存対象、再開状態を固定。旧AGENTS/MEMORYルーティング指示は撤回に合わせ削除。アプリ実装はまだ起動しない。
