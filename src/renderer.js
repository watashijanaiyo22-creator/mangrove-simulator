...
export class Renderer {
 static async create(device,canvas,sim,forest){
  const r=new Renderer();Object.assign(r,{device,canvas,sim,forest,textures:[],buffers:[],crown:true});r.home();
  ...
  const instances=new Float32Array(forest.trees.flatMap(t=>[t.x,t.y,t.z,t.scale,t.angle,0,0,0]));r.instances=r.makeBuffer(128*32,GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST,instances);
 ...
 setForest(forest){this.forest=forest;this.device.queue.writeBuffer(this.instances,0,new Float32Array(forest.trees.flatMap(t=>[t.x,t.y,t.z,t.scale,t.angle,0,0,0])));}
 home(){this.angle=-2.8;this.elevation=.52;this.distance=58;this.target=[44,.3,23];}
